...
| `reflexes.py` | `ReflexEngine` — Canny edge density check on the near-field of each frame; triggers an evasive turn if an obstacle is detected directly ahead. No API call. **Currently unused** — `vision_loop.py` doesn't call it (see below); kept in case obstacle avoidance gets reintroduced. |
...
| `vision_loop.py` | `VisionControlLoop` — single-threaded, exactly three modes. Every pan-tilt move is routed through `_move_pan`/`_center_camera`, the only two places in the code allowed to call the servo API — both force `motor.stop()` and settle immediately beforehand, unconditionally, so motors can never be running while the camera pans (hard rule). **SEARCHING**: L298N fully disabled for the entire mode — all six pins LOW including ENA/ENB (`MotorController.disable()`), driver outputs off while servos are active; drive commands re-enable automatically. Pan-tilt sweeps 5 fixed angles (30/60/90/120/150°) asking Claude "is the shoe here?" at each; if the whole sweep misses, it just sweeps again from the first angle. **ALIGNING**: pan-tilt re-centers to forward first, then the car body turns once to face the direction the shoe was found in (turn duration scaled from the pan offset), then takes a fresh photo — found in frame -> APPROACHING, not found -> back to SEARCHING. **APPROACHING**: drives straight forward until interrupted (Ctrl+C or `--iterations`); no steering or obstacle checks. Claude only ever reports structured observations (`FOUND`, `POSITION`) — it never picks a direction, motor/servo actions are derived deterministically in code. |
...
| Pin 16 | GPIO23 → IN1 (right forward) — moved from Pin 12/GPIO18 on 2026-07-10, see incident note below; GPIO18 is retired, leave it unused |
...
|---|---|
| Servo board power (+) | **Pin 2** (5V) — moved from Pin 4 on 2026-07-13; Pin 4's contact is worn out, do not use it for anything critical |
| Servo board GND | **Pin 6** (GND) — moved from Pin 9 on 2026-07-13; Pin 9's contact is suspect, avoid |
...
- Vision loop pipeline: exercised in simulation mode locally (mock camera, dummy key, simulated servos/motors, mocked motor/servo calls recorded and inspected) — confirmed the drive motors are never called during SEARCHING (only `stop()`, once per pan move), the full-sweep-miss → re-sweep path, the align → turn → confirmation-photo → approach path, and the confirmation-photo-fails → back-to-search path. Not yet run on real hardware or stress-tested for extended real-world runs.
...
2. **Alternated servo/motor test** — pan → settle → drive → stop → pan the other way → settle → drive → stop, checking `vcgencmd get_throttled` throughout. Validates the hard-rule sequencing under realistic combined usage. The ALIGNING → APPROACHING transition in `vision_loop.py` does one instance of this pattern in production (pan-tilt recenters, then the car turns once), so this test doubles as validation for it.
...
- **Right-wheels-spinning incident (2026-07-10, RESOLVED — root cause: faulty L298N module internally shorting its own 5V logic rail to IN1.** Symptom: right wheels spun continuously during the search phase even though the log showed only `stop` commands. Diagnosis path, in order: (1) `[MOTOR]`/`[PINCHECK]` logging in `motor_control.py` proved software commanded LOW while the pin measured HIGH; (2) internal pull-down test still read hi → real short, not leakage; (3) pin read lo with the wire unplugged → GPIO18 silicon works; (4) replacing the wire changed nothing; (5) a fresh pin (GPIO23) driving the same line also read hi → no GPIO can sink it, source is low-impedance 5V; (6) unpowering the servo board changed nothing; (7) **disconnecting the L298N's +5V logic wire made the line read lo → the 5V was flowing out of the module's own IN1 pin.** The short developed during the pan-tilt install (mechanical chassis work — likely a bridge/debris under the board), since all four directions had tested clean before it. Fix: repair or replace the L298N (~$4). Fallout: IN1 was remapped from GPIO18/pin 12 to **GPIO23/pin 16** — GPIO18 sank a hard 5V short for an extended period and is retired as a precaution (it passed a light-load test but is not trusted for anything that matters). The `[PINCHECK]` readback (pinctrl-based, runs after every motor command) is permanent — a `MISMATCH` error in the log means this class of fault has returned. Notes for next time: Pi 4's drive register is write-only (`pinctrl` shows `--` for drive), so use the pull-down test to distinguish short from leakage; undriven motor lines float HIGH on this build, so a disconnected IN wire = that motor runs — don't leave IN wires dangling with the battery connected.

## Dead hardware awaiting replacement (as of 2026-07-11)

1. **L298N motor driver** — internal short from its own 5V logic rail to IN1 (right forward). Confirmed by: line reads 5V-hard against a healthy GPIO driving low, clears the moment the module's 5V feed is disconnected. Replace with **Cytron MDD3A** (preferred: 3A/channel, 2.5-9.5V, protected, onboard test buttons) or TB6612FNG. `motor_control.py` needs a small rewrite for the new board's control scheme when it arrives. Note: IN1's Pi end is now **GPIO23/pin 16** (GPIO18 retired).
2. **RESOLVED 2026-07-13 — the "dead PCA9685" verdict was WRONG. Root cause: worn Pi header contacts on pins 4 (5V) and 9 (GND).** Both the original Arducam board and a brand-new replacement showed identical symptoms (address ACKs on scans, all data transfers fail, chip stuck at power-on defaults 0x11) because the chip was continuously brown-out resetting on flaky power — not because either board was damaged. Proven by: new board + fresh wires still failed; GPIO2/3 drive-tested healthy; bit-banged I2C on the same pins failed identically; L298N fully removed changed nothing; a clean 30/30 window appeared right after connectors were physically handled then collapsed — classic failing contact; moving V+ to pin 2 and GND to pin 6 fixed it instantly and permanently (20/20 bursts at boot/2min/5min, then full `test_pan_tilt.py` pass with servos moving and zero throttle events). **The original Arducam board is probably fine — test it as a spare** with `i2cget -y 1 0x40 0x00 b` on the new pin wiring. Lesson: header pins wear out after dozens of plug cycles; when a "dead" I2C device ACKs scans but fails data with MODE1 at power-on defaults, suspect power delivery before silicon.

**Wiring discipline going forward: to unpower an I2C device, disconnect ALL of its wires, not just power — idle SDA/SCL still carry 3.3V through the Pi's pull-ups. GND connects first and disconnects last; never plug/unplug anything with the Pi or battery powered. And minimize header plug cycles — the contacts are only rated for so many insertions.**
