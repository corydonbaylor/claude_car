...
- **Right-wheels-spinning incident (2026-07-10): the IN1 line (GPIO18, physical pin 12) was shorted to 5V externally.** Symptom: right wheels spun continuously during the search phase even though the log showed only `stop` commands. Diagnosis path, in order: (1) `[MOTOR]`/`[PINCHECK]` logging in `motor_control.py` proved software commanded LOW while the pin measured HIGH; (2) pull-down test (`GPIO.PUD_DOWN` still read hi → external pull stronger than ~50kΩ, i.e. a real short, not leakage); (3) unplugging the IN1 jumper from pin 12 made the pin read lo → GPIO18 silicon healthy, short is in the wire or at the L298N end. The `[PINCHECK]` readback (pinctrl-based, runs after every motor command) is permanent — a `MISMATCH` error in the log means this class of fault has returned. Note: Pi 4's drive register is write-only (`pinctrl` shows `--`), so commanded-vs-measured can't be read from pinctrl alone on this hardware; use the pull-down test to distinguish a short from leakage. Also learned: undriven motor lines float HIGH on this build, so a disconnected IN wire = that motor runs; don't leave IN wires dangling with the battery connected.
